...
def get_order_data():
    ...
    d = {
      "16oz Coffee Cup": [2345, 2.79, 'Pack of 25'],
    ...
    while True:
        ...
        print ("{:<25} {:<10} {:<15} {:<10}\
        ".format('Name', 'SKU', 'Price exVAT', 'Shipped'))
        for name, v in d.items():
            sku, price, ship = v
            print ("{:<25} {:<10} {:<15} {:<10}\
            ".format(name, sku, price, ship))
        print("\n")
        print("==============================\n")
        ...
        payment_method = input(
            ...
            "Bank Transfer or 'p' = Pay on Account:\n"
        )
        date_stamp_preview = (
            f"Order Date: {datetime.datetime.now():%d-%m-%Y}"
        )
        ...
        """
        To assign the product names to the order
        based on sku selected by the user input.
        """
        if product_sku == '2345':
        ...
        """
        To assign the payment method string sentence
        to the order based user input.
        """
        if payment_method == 'b':
        ...
        order_data = (
            date_stamp_file, user_name, branch_number, product_sku,
            productName, product_qty, productPrice, paymentMethod
        )

        """
        If the user does not confirm the oder, the user is sent back\
        to the beginning of the program. Order is not submitted.
        """
        if confirm_order != 'y':
            print(
